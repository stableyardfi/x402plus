export type ExactAccepts = {
  ...
  asset: string;
  extra?: { name?: string; version?: string } | Record<string, any>;
};
...
export type Signer = {
  signExact: (accepts: ExactAccepts) => Promise<string>;  // returns base64 X-PAYMENT
  signIntent: (accepts: IntentAccepts, hint?: any) => Promise<string>; // returns base64 X-PAYMENT
};
...
async function ensureNetwork(network: string, ethereum: any) {
...
export function evmInjectedSigner(ethereum: any): Signer {
  return {
    async signExact(accepts: ExactAccepts): Promise<string> {
      ...
      const domain = {
        name: (accepts.extra as any)?.name || "USDC",
        version: (accepts.extra as any)?.version || "2",
     ...
     */
    async signIntent(accepts: IntentAccepts, hint?: any): Promise<string> {
      if (!ethereum) throw new Error("No injected wallet");
      const accounts = await ethereum.request({ method: "eth_requestAccounts" });
      const from = accounts[0];
      await ensureNetwork(accepts.network, ethereum);
...
export function wrapFetchWithPayment(baseFetch: typeof fetch, opts: ClientOptions) {
  ...
  return async function fetchWithPayment(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    const xPayment = (scheme === "intent")
      ? await opts.signer.signIntent(accepts, (accepts as any)?.extra?.payload)
...
// ---- Aptos-like (Movement-compatible) signer helpers ----
// These helpers let callers plug in any Aptos/Movement signer (wallet or SDK)
// by returning base64-encoded BCS bytes for the authenticator and raw transaction.
