...
function isMovementNetwork(network) {
...
app.use((req, res, next) => {
    ...
    if (req.body && Object.keys(req.body).length > 0) {
        ...
        if (bodyLog.paymentPayload?.signature)
            bodyLog.paymentPayload.signature = bodyLog.paymentPayload.signature.slice(0, 20) + "...";
...
function stableyardBase() {
    const base = (process.env.STABLEYARD_BASE_URL || "").replace(/\/$/, "");
    if (!base)
        throw new Error("STABLEYARD_BASE_URL is not configured");
    return base;
}
...
async function syGetUserByAddress(address) {
...
async function syRegisterUser(address, chain, token) {
...
async function syCreateVault(userId) {
    ...
    const accountAddress = (data?.vaultAddress || data?.safeAddress || null);
...
async function syGetBalance(userId) {
...
async function syGenerateDepositAddress(userId, chainId) {
...
async function syGetQuote(userId, partnerId, amount, destinationChainId, destinationToken) {
    const body = { userId, partnerId, amount };
    if (destinationChainId)
        body.destinationChainId = destinationChainId;
    if (destinationToken)
        body.destinationToken = destinationToken;
...
async function ensureUserAndAccount(address, chain, token) {
    ...
    let account = { activated: false, accountAddress: null, signing: null };
    if (!accountAddress) {
...
app.post("/settle", async (req, res) => {
    ...
    try {
        ...
        if (scheme === "exact") {
            ...
            try {
                const tx = await contract.transferWithAuthorization(payer, // from (address)
                merchant, // to (address)
